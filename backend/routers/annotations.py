...
@router.get("/{task_id}", response_model=list[AnnotationResponse])
...
def get_annotations_for_image(image_id: int):
    with get_db() as db:
        rows = db.execute(
            "SELECT * FROM annotations WHERE image_id = ? ORDER BY user_id, layer_index",
            (image_id,),
        ).fetchall()
        return [dict(row) for row in rows]
